...
//     let response = await (mockRequest.post('/animal').send(data));
...
//     let response = await mockRequest.get('/animal');
//     expect(response.status).toBe(200);
//     expect(response.body).toBeInstanceOf(Array);
//     expect(response.body.length).toBeGreaterThan(0);
//     expect(response.body[0]).toHaveProperty('firstName');
//     // expect a list of animal in the body...
//   });

//   it('can get a record', async () => {
//     let response = await mockRequest.get('/animal/1');
//     expect(response.status).toBe(200);
...
//     let data = {};
//     let response = await mockRequest.put('/animal/1',data);
//     expect(response.status).toBe(200);
//     // expect the right record in the body...
//   });
...
//     // Delete that record
//     let deleteResponse = await mockRequest.delete(`/animal/${id}`);
